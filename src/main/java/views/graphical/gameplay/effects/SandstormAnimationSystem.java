...
public final class SandstormAnimationSystem {
    ...
    private static final float SANDSTORM_OFFSET_X = 0f;
    private static final float SANDSTORM_OFFSET_Y = 10f;

    private final Map<Zombie, SandstormVisual> visuals =
        new IdentityHashMap<>();

    public SandstormAnimationSystem(
        PamPlayer pamPlayer,
        Stage worldStage,
    ...
    public SandstormAnimationSystem(
        ...
        this.pamPlayer = Objects.requireNonNull(
            pamPlayer,
            "pamPlayer"
        );

        this.worldStage = Objects.requireNonNull(
            worldStage,
            "worldStage"
        );
        ...
        this.theme = Objects.requireNonNull(
            theme,
            "theme"
        );
        ...
        if (theme == ChapterTheme.ANCIENT_EGYPT) {
    ...
    public void update(
        ...
        for (Zombie zombie : safeZombies) {
            ...
            SandstormTransportBehavior transport =
                zombie.getBehavior(
            ...
            if (transport.isActive()
                ...
                visual.follow(zombieActor);
            } else if (visual != null) {
                visual.requestStop();
            }
        }

        Iterator<Map.Entry<Zombie, SandstormVisual>> iterator =
            visuals.entrySet().iterator();

        while (iterator.hasNext()) {
            Map.Entry<Zombie, SandstormVisual> entry =
                iterator.next();

            Zombie zombie = entry.getKey();
            SandstormVisual visual = entry.getValue();
    ...
    public void clear() {
        for (SandstormVisual visual : visuals.values()) {
    ...
    private SandstormVisual createVisual() {
        ...
        rear.setScale(scale, scale);
        top.setScale(scale, scale);

        worldStage.addActor(rear);
        worldStage.addActor(top);

        rear.restart();
        top.restart();

        float introDuration = Math.max(
            safeDuration(REAR_PAM, INTRO, 0.20f),
            safeDuration(TOP_PAM, INTRO, 0.20f)
        );

        float outroDuration = Math.max(
            safeDuration(REAR_PAM, OUTRO, 0.20f),
            safeDuration(TOP_PAM, OUTRO, 0.20f)
        );

        return new SandstormVisual(
            rear,
            top,
            introDuration,
            outroDuration
    ...
    private final class SandstormVisual {
        ...
        private void follow(Actor zombieActor) {
            if (finished || zombieActor == null) {
                return;
            }
            ...
            rear.setPosition(x, y);
            top.setPosition(x, y);

            updateLayering(zombieActor);
        }
        ...
        private void playOutro() {
            ...
            rear.play(OUTRO, false);
            top.play(OUTRO, false);

            rear.restart();
            top.restart();
        }

        private void updateLayering(Actor zombieActor) {
            if (zombieActor == null
                || zombieActor.getParent() == null
                || rear.getParent() != zombieActor.getParent()
                || top.getParent() != zombieActor.getParent()) {
                return;
            }

            Group parent = zombieActor.getParent();

            int targetIndex = zombieActor.getZIndex();
            ...
            targetIndex = zombieActor.getZIndex();
