...
public final class PlantSlotsBar extends Table {
    ...
    private final ButtonGroup<PlantCard> gameplayPlantGroup = new ButtonGroup<>();
    ...
    private final Map<Integer, PlantCard> gameplayCardsByPlantId = new HashMap<>();
    private final Map<Integer, Integer> cooldownStartTickByPlantId = new HashMap<>();
    private final Map<Integer, Integer> cooldownEndTickByPlantId = new HashMap<>();
    ...
    public void loadPlants(List<PlantData> plants) {
        ...
        if (plants != null) {
            int count = Math.min(plants.size(), MAX_SLOTS);
    ...
    public boolean contains(PlantData plant) {
        if (plant == null) {
            return false;
        }
    ...
    public int addPlant(PlantData plant) {
    ...
    public PlantData getPlant(int slotIndex) {
        checkSlotIndex(slotIndex);
        return slots[slotIndex];
    }

    public void removePlant(int slotIndex) {
        checkSlotIndex(slotIndex);
                ...
                card.addListener(new ClickListener() {
                    ...
                    public void clicked(InputEvent event, float x, float y) {
                        if (onRemoveRequested != null) {
                            onRemoveRequested.accept(slotIndex);
                        }
                    }
                });
            }
    ...
    private void refreshGameplayAvailability() {
        ...
        for (PlantData plant : slots) {
            ...
            PlantCard card = gameplayCardsByPlantId.get(plant.id());
            if (card == null) {
                continue;
            }

            int cooldownEnd = state.getPlantCooldownEnd(plant.id());
            int ticksRemaining = Math.max(0, cooldownEnd - currentTick);

            if (ticksRemaining > 0) {
                Integer knownEnd = cooldownEndTickByPlantId.get(plant.id());
                if (knownEnd == null || knownEnd != cooldownEnd) {
                    cooldownEndTickByPlantId.put(plant.id(), cooldownEnd);
                    cooldownStartTickByPlantId.put(plant.id(), currentTick);
                }

                int cooldownStart = cooldownStartTickByPlantId.getOrDefault(plant.id(), currentTick);
                int totalTicks = Math.max(1, cooldownEnd - cooldownStart);
                float fraction = Math.min(1f, ticksRemaining / (float) totalTicks);
                card.setCooldownFraction(fraction);
            } else {
                cooldownStartTickByPlantId.remove(plant.id());
                cooldownEndTickByPlantId.remove(plant.id());
                card.setCooldownFraction(0f);
            }

            boolean enoughSun = currentSun >= plant.cost();
            boolean coolingDown = ticksRemaining > 0;

            card.setEnoughSun(enoughSun);
            card.setAvailable(enoughSun && !coolingDown);
        }
    }

    private Image createEmptySlot() {
        TextureRegion region = game.getTextureBank().region(EMPTY_PACKET);
        if (region == null) {
            throw new IllegalStateException("TextureBank region was not found: " + EMPTY_PACKET);
        }

        Image emptySlot = new Image(new TextureRegionDrawable(region));
        emptySlot.setScaling(Scaling.stretch);
        emptySlot.setTouchable(Touchable.disabled);
        return emptySlot;
    }
    ...
    private void checkSlotIndex(int slotIndex) {
        if (slotIndex < 0 || slotIndex >= MAX_SLOTS) {
            throw new IndexOutOfBoundsException("slotIndex must be between 0 and " + (MAX_SLOTS - 1));
        }
    }
}
