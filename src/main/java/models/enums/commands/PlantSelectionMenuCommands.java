...
import views.PlantSelectionMenu;
...
public enum PlantSelectionMenuCommands implements Commands{
    ...
    ADD_PLANT_REGEX("^\\s*remove\\s+plant\\s+-t\\s+(?<type>\\S+)\\s*&"),
    REMOVE_PLANT_REGEX("^\\s*remove\\s+plant\\s+-t\\s+(?<type>\\S+)\\s*$"),
    BOOST_PLANT_REGEX("^\\s*boost\\s+plant\\s+-t\\s+(?<type>\\S+)\\s*$"),
    ...
    PlantSelectionMenuCommands(String regex) {this.regex = regex;}
    @Override
